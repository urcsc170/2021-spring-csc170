...
- You must *try* to maintain the integrity of each HTML document, however you will find you might have to move things around - restructure the content - whatever it takes to make your website work logically and consistently, from page to page
  - You may find it necessary to remove internal navigation (if any), and resize or crop images
  - In some cases, you might need to go out to the web and find more content - typically images - to create some consistency ...it's up to you
...
**It is your responsibility to obtain original Project 1 documents (HTML and image files) from other students using the *scraping* method.**  

## Validation

When you turn-in your Project 3, all HTML and CSS files must validate, however there are known errors that can be excused.  If you have questions about excusable errors, contact your TA or the professor.  

Also: *warnings* do not have to be fixed, but you should consider whatever the W3C tells you

## Responsive/Adaptive Design

Note: when starting Project 3 you will not be able to complete this section until *after* we cover the topic on **Mobile Design**, later in the semester.

-  Every page on the website must display properly on all devices

  - The website design must work at browser-widths down to mobile-size (480px wide), and up to large desktops (1200px wide), and everything in between
  - On small devices: there must be **no horizontal scrolling** and **no elements that overlap** their borders (unless it is done for artistic purposes)
  - On desktops, there must be no paragraphs of normal-sized text (16px) that stretch more than about 700px wide (it's a readability issue)
...
Each page in your website must have the following items **in common**, meaning that these items must be identical on each page in the website:

- A banner (in a HEADER element)
- A main menu (in a NAV element)
- A footer (in a FOOTER element)
- The overall page layout using CSS Grid Layout
